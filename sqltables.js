...
connection.query(
    'CREATE TABLE if not exists products(id INT NOT NULL AUTO_INCREMENT, category VARCHAR(20),brand VARCHAR(50), name VARCHAR(150),specifications TEXT, images TEXT, price INT(10), rating INT DEFAULT 0, reviews INT DEFAULT 0, quantity INT DEFAULT 1, datePosted DATETIME DEFAULT CURRENT_TIMESTAMP,status VARCHAR(25) DEFAULT "IN-STOCK", PRIMARY KEY (id))',
    (error,result)=>{
        if(error){
            console.log(error)
        }
...
connection.query(
    'CREATE TABLE if not exists tempcart(id INT NOT NULL AUTO_INCREMENT, userId TEXT, productId INT, productName VARCHAR(115),quantity INT DEFAULT 1, PRIMARY KEY (id), FOREIGN KEY (productId) REFERENCES products(id))',
    (error,result)=>{
        if(error){
            console.log(error)
        }
